...
## #028 新增远程 embeddings provider（OpenAI 兼容 / BAAI-bge-m3 dense）
**日期**：2026-03-09

**目标**：
- 保留原有 `local` / `mock`
- 新增 `remote` provider
- 插件侧按 OpenAI 兼容 embeddings API 接入 `BAAI/bge-m3`
- 仅支持 dense embedding，不引入 sparse / ColBERT / multi-vector

**实现结果**：
1. 新增 `src/embeddings/remote-provider.ts`，复用现有 `EmbeddingProvider` 抽象。
2. 远程请求格式固定为：
   - `POST {baseUrl}/v1/embeddings`
   - `Authorization: Bearer {apiKey}`
   - `Content-Type: application/json`
3. 默认模型名为 `BAAI/bge-m3`。
4. 支持单条与批量 embedding；批量调用会按设置中的 `Batch Size` 自动分批。
5. 向量维度不再写死；虽然 `bge-m3` dense 常见是 1024 维，但插件以接口真实返回为准。
...
其中 `Test Connection` 会真实发一次 embeddings 请求，不是本地校验假按钮。

**索引与快照行为**：
1. 从 `local/mock` 切到 `remote` 时，仍会清空旧索引并提示手动“重建索引”。
2. 索引快照现在记录并检查：
   - `embeddingProvider`
   - `embeddingDimension`
   - `remoteModel`
3. 当前实现还额外记录 `remoteBaseUrl`，防止更换远端服务后继续误用旧快照。
4. 若当前 provider 为 `remote` 且 `Base URL` / `API Key` 未填，启动阶段会跳过自动重建。
...
- 不下载本地模型
- 不依赖 `models/` 本地缓存
- 不受 `forcePluginLocalModelStorage` 影响
- 不做 sparse embedding
...
**验证**：
1. 设置页可切换到 `remote`。
2. 可填写 `Base URL / API Key / Model / Timeout / Batch Size`。
3. `Test Connection` 能返回成功或明确失败原因。
4. `rebuild index` 会实际调用远程 embeddings。
5. 快照保存与加载不会因 `remote` provider 崩溃。

**状态**：已实现；是否能真正连通取决于用户后续填写的真实 API 地址与 Key。
---
