...
**问题分析**：
- 如果 API Key 失效导致 1000 个文件全都失败，30 天内日志会暴增到几千条。
...
### 问题 D：远端 API 返回值校验不足
`RemoteProvider` 原先没有校验返回条数、`index` 连续性、重复索引和向量维度一致性，兼容性差的 OpenAI 风格服务可能返回部分坏数据并被继续写入索引。
...
4. `RemoteProvider` 对返回结果做完整性校验，不合法直接报错。
5. 快照恢复失败时同步清空 `noteStore`、`chunkStore`、`vectorStore`，避免脏状态。
...
- `README.md` — 更新用户侧说明
- `ARCHITECTURE.md` — 更新主线程 / worker 分工说明
...
- `src/settings.ts` — 远程模型列表获取失败落盘
- `src/embeddings/embedding-service.ts` — 返回远程模型列表失败的诊断信息
- `src/utils/error-logger.ts` — 错误日志文件自身加载失败时的自诊断
- `src/types.ts` — 扩展 `errorType` 类型
- `README.md` — 更新用户侧错误日志说明
- `ARCHITECTURE.md` — 更新错误日志覆盖范围说明
...
- `ARCHITECTURE.md` — 更新 LocalProvider 的运行模型说明
...
- `README.md` — 更新本地模型运行方式说明
- `ARCHITECTURE.md` — 更新 LocalProvider 降级链说明
